...
namespace _24Point
{
    public partial class Form1 : Form
    {
        ...
        {
            lblInput.Text = "";
            NumberA = new Random(Guid.NewGuid().GetHashCode()).Next(1, 11);//牌面大小对应的数字大小
            btnNumber1.Text = NumberA.ToString();
            //画第二张牌
            NumberB = new Random(Guid.NewGuid().GetHashCode()).Next(1, 11);
            btnNumber2.Text = NumberB.ToString();
            //画第三张牌
            NumberC = new Random(Guid.NewGuid().GetHashCode()).Next(1, 11);
            btnNumber3.Text = NumberC.ToString();
            //画第四张牌
            NumberD = new Random(Guid.NewGuid().GetHashCode()).Next(1, 11);
        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            ...
            if (CheckForNumber(lblInput.Text.Trim()))//检查输入表达式中输入的数字是否匹配
            {
                ...
                if (result == 24)
                {
                    txtAnswer.Text = "<<---恭喜您，答对了！--->>";
                    timer1.Enabled = false;//暂停时钟
                }
                else
                {
                    txtAnswer.Text = "<<--抱歉，您的回答有误!-->>";
                }
            }
        }
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            ...
            #endregion
            //画第一张牌      
            NumberA = new Random(Guid.NewGuid().GetHashCode()).Next(1, 11);
            btnNumber1.Text = NumberA.ToString();
            //画第二张牌           
            NumberB = new Random(Guid.NewGuid().GetHashCode()).Next(1, 11);
            btnNumber2.Text = NumberB.ToString();
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("做不出来请点击查看按钮，查看本轮答案", "时间警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
